...
Subscript property access and happens *after* prefix increment `++` kicks in. As `f` doesn't contain a property `z`, it returns `undefined`, which is *falsey*, so , `|| 1` is the result. Therefore, assign 1 to `f['z']`.

Next time, a similar thing happens. `f['z']` evals to 1, so, `++f['z']`  increments 1 to 2, and assigns 2 to `f['z']`. And so on and so forth.