...
Next time, a similar thing happens. `f['z']` evals to 1 (it is not `undefined` any longer)p, so, `++f['z']`  increments 1 to 2, and assigns 2 to `f['z']`. And so on and so forth.