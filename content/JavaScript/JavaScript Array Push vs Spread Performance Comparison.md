...
description: Some notes, tips and examples on the JavaScript performance on arrays when using .push() vs spread to make copies.
---
## Introduction

![JavaScript performance Top Gear I game screenshot](../__assets/top-gear-game-kph-mph.png)

We all (or most of us) like to use fancy new language features. Except sometimes the new, fancy features are not the best choice for a given situation, and using some older (or less elegant, or frowned upon) feature ends up yielding better performance results.
...
With the _spread_ approach, we **always create a new array**, **copy the current existing values to it** and add the new value. And it does this each time (or each iteration of the `reduce`), with ever increasing number of values to copy. Let me repeat: it copies and recopies an ever increasing number of values.
