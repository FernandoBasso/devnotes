...
At some point, and if the two conditions have always been satisfied, there will be a single remaining element on the vector. This is the base case where we return `true`.

All other cases mean some condition was not not satisfied so we return `false`.

### odd-week? with partial application

I have seen some other solutions on Exercism where they use `partial`. It seems to work because the “odd week” is supposed to be precisely `[1 0 1 0 1 0 1]`. It seems it would not possibly be `[0 1 0 1 0 1 0]`.
